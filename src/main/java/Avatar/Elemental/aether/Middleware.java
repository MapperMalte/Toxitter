...
public abstract class Middleware
{
    ...
    public static String extractGetParam(String uri, String param)
    {
        ...
        if (BookOfIlaan.active) BookOfIlaan.write(ToxitterSecurityMiddleware.class,"Extracted token "+token+" from Uri: "+uri);
        uri = uri.substring(0,beginTokenParamIndex)+uri.substring(endTokenIndex,uri.length());
        if (BookOfIlaan.active) BookOfIlaan.write(ToxitterSecurityMiddleware.class,"Now uri: "+uri);
        return token;
    }
