...
import Avatar.Elemental.fire.artifacts.test.CostFunction;
...
public class FireOfSamrosh extends ReservoirEntity
{
    ...
    public void init_Homunculus(int size, int inputDimension, int outputDimension)
    {
        ...
        input_hiddenLayer_output_FromToConnectionWeights = new WeightSeeder().seedRandomTensor(input_activation_output_layers_length);
    ...
    public double[] propagateForward(double[] inputVector, double[] correctOutput, boolean debug)
    {
        ...
        for (int currentLayerOfDeepNeurons = 0; currentLayerOfDeepNeurons < numberOfHiddenLayers; currentLayerOfDeepNeurons++)
        {
            ...
            for(int weightIndex_currentLayer = 0; weightIndex_currentLayer < hidden_layers_length[currentLayerOfDeepNeurons]; weightIndex_currentLayer++)
            {
                ...
                input_activation[currentIndexOf_Input_Activation_Output][weightIndex_currentLayer] = FastGauß.getErf(input_activation[currentIndexOf_Input_Activation_Output][weightIndex_currentLayer]);
            }
        }
        ...
        {
            DELTA_IN_NEURON_PLUS_OUTPUT[numberOfHiddenLayers][x] = (output[x]-correctOutput[x]);
        }
        ...
        for(int currentLayer = numberOfHiddenLayers-1; currentLayer >= 0; currentLayer--)
        {
            ...
            for(int indexInCurrentLayer = 0; indexInCurrentLayer < input_activation_output_layers_length[currentLayer]; indexInCurrentLayer++)
            {
                ...
                DELTA_IN_NEURON_PLUS_OUTPUT[currentLayer][indexInCurrentLayer]*=BookOfRass.getSigmoidDerivative(input_activation[currentLayer][indexInCurrentLayer]);
